...
void printHelp()
{
   const char * helpMsg = "function [] = TwoTubeMexWithStabilityComp(T1,T2,psiL,Beta)   \n"
                          "   Inputs:\n"
...
void copy_outputs_to_matlab( KinRetDense<State<2,OType> > const &ret1, mxArray *plhs[] )
{
    ...
    // ptip = p_w_b or p from world frame to body/tip or position of the tip in the world frame
	std::copy( ret1.pTip.data(), ret1.pTip.data() + 3, pdata ); 

    // qtip = q_w_b or q from world frame to body/tip or rotation of the tip in the world frame
...
// 		transformation g_star = g_base * transformation(pi,qi);
		transformation g_star = transformation(pi,qi);
	...
	Eigen::Matrix<double,6,6> Ad_beta = Utility::Adjoint_p_q(ret1.y_final.p, ret1.y_final.q); // adjoint matrix from world to body frame
	Eigen::Matrix<double,6,4>	const &Zga_beta = ret1.y_final.Zga; // tip jacobian in body frame
	for (int i = 0; i < Npts; ++i) 
	{
		mxArray *Ji = mxCreateDoubleMatrix(6, 4, mxREAL);
		double *JiData = mxGetPr(Ji);	
		Eigen::Matrix<double, 6, 4> const &Zga_i = ret1.dense_state_output.at(i).Zga; // gets body jacobian at the arclength i
		Eigen::Vector3d p_inv;
		Eigen::Vector4d q_inv;

        // invert the transformation from body (tip) to point i -> T from i to body (tip)
		transformation_tie(p_inv, q_inv) = transformation_inverse( ret1.dense_state_output.at(i).p, ret1.dense_state_output.at(i).q );
		Eigen::Matrix<double,6,6>	Ad_s = Utility::Adjoint_p_q( p_inv, q_inv ); // adjoint of T from i to body frame

		Eigen::Matrix<double,6,4> Jgi = Zga_i - Ad_s * Ad_beta * Zga_beta; // no clue whats happening here
