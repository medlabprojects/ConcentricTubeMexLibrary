...
#define PROGRAM_NAME "TwoTubeMexMultiCurve"
...
typedef smooth_step_function<Eigen::Vector2d>   CurvFun;
...
void printHelp()
{
   const char * helpMsg = "function [] = TwoTubeMexWithPsiMultiCurve(T1,T2,psiL,Beta)   \n"
                          "   Inputs:\n"
...
bool checkTube( const mxArray *tube )
{
   ...
   if ( mxGetNumberOfFields( tube ) != 9 ) return false; //tubes need 7 pieces of information (see help info above)

   if ( mxGetField( tube, 0, "OD" ) == 0 ) return false;
   if ( mxGetField( tube, 0, "ID" ) == 0 ) return false;
   if ( mxGetField( tube, 0, "k1" ) == 0 )  return false;
   if ( mxGetField( tube, 0, "k2" ) == 0 )  return false;
   if ( mxGetField( tube, 0, "L" ) == 0 )  return false;
   if ( mxGetField( tube, 0, "Lc_transition" ) == 0 )  return false;
...
{
   double ODval, IDval, k1val, k2val, Lval, Lc_transition_val, Ltval, Eval, Gval;
   ...
   Ltval = *mxGetPr( Lt );
   mxArray *Lc_transition = mxGetField( tube, 0, "Lc_transition" );
   Lc_transition_val = *mxGetPr( Lc_transition );
   mxArray *E = mxGetField( tube, 0, "E" );
   Eval = *mxGetPr( E );
   mxArray *G = mxGetField( tube, 0, "G" );
   Gval = *mxGetPr( G );
   mxArray *k1 = mxGetField( tube, 0, "k1" );
   k1val = *mxGetPr( k1 );
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )
{
   ...
   if ( nlhs == 1 )
   {
      const char *fields[] = { "p_tip", "q_tip", "J_tip" ,"s", "p", "q", "J", "psi" };
      size_t dims[1] = { 1 };
