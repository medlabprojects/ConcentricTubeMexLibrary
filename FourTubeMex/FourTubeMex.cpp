...
#define PROGRAM_NAME "FourTubeMex"
...
typedef std::tuple< Tube<CurvFun>, Tube<CurvFun>, Tube<CurvFun>, Tube<CurvFun> > CannulaT;
...
//, Option::ComputeCompliance, Option::ComputeStability,
//                        Option::ExternalLoads

void printHelp()
{
   const char * helpMsg = "function [] = FourTubeMex(T1,T2,T3,T4,psiL,Beta)   \n"
                          "   Inputs:\n"
                          ...
                          "      T4:   Tube structure for tube 4 (see definition below)\n"
...
bool checkConfig( const mxArray *config )
{
   ...
   if ( mxGetNumberOfElements( config ) != 4 ) return false;
   return true;
}
...
Configuration GetConfigFromInput( const mxArray *psiL, const mxArray *Beta )
{
   ...
   Configuration q;
   q.PsiL << psiL_data[0], psiL_data[1], psiL_data[2], psiL_data[3];
   q.Beta << Beta_data[0], Beta_data[1], Beta_data[2], Beta_data[3];
   q.Ftip << 0.0, 0.0, 0.0;
   q.Ttip << 0.0, 0.0, 0.0;
   return q;
}
...
//   }
   ...
   if ( nlhs == 1 )
   {
      const char *fields[] = { "p_tip", "q_tip", "J_tip", "C_tip", "s", "p", "qq", "stability"}; //do stability later
      size_t dims[1] = { 1 };
      plhs[0] = mxCreateStructArray( 1, dims, 8, fields );

      using namespace CTR;
      Tube<CurvFun> t1 = GetTubeFromInput( prhs[0] );
      Tube<CurvFun> t2 = GetTubeFromInput( prhs[1] );
      Tube<CurvFun> t3 = GetTubeFromInput( prhs[2] );
      Tube<CurvFun> t4 = GetTubeFromInput( prhs[3] );

      CannulaT cannula = std::make_tuple( t1, t2, t3, t4 );
      Configuration q = GetConfigFromInput( prhs[4], prhs[5] );

      KinRetDense<State<4,OType> > ret1 = Kinematics_with_dense_output( cannula, q, OType() );

      mxArray *p_tip = mxCreateDoubleMatrix( 3, 1, mxREAL );
      mxArray *q_tip = mxCreateDoubleMatrix( 4, 1, mxREAL );
      mxArray *J_tip = mxCreateDoubleMatrix( 6, 8, mxREAL );
      ...
      Eigen::Matrix<double, 6, 8> J = GetTipJacobianForTube1( ret1.y_final );
      std::copy( J.data(), J.data() + 48, Jdata );

      Eigen::Matrix<double, 6, 6> C = GetTipComplianceForTube1( ret1.y_final );
      //Eigen::Matrix<double,6,6> C; C.fill(0.0);
